...
def main():
	...
	WITH client_azs AS (
		...
		WHERE CC.status IN('trial', 'demo', 'client')),
		competitors_azs AS (
	...
	AND ((GSP.show_in_report = 't' AND DATE(GSP.updated_at) > CURRENT_DATE-60) OR (GSP.show_in_report = 'f' AND 
		DATE(GSP.updated_at) > CURRENT_DATE-7))
	ORDER BY 1, 5, 7, 2
	"""
	...
	df_1 = df01.loc[
		(df01['source'].isin(
			['api_azsgo', 'api_azsopti', 'api_yandex_fuel', 'api_benzuber', 'api_yandex_other', 'manual_app',
			 'manual_manual', 'manual_phone', 'manual_ppr', 'manual_gazprom', 'manual_other']))
	...
	source_dict = {'api_azsgo': 1, 'api_azsopti': 2, 'api_benzuber': 3, 'api_yandex_fuel': 4, 'api_rncard': 5,
	...
	df17 = pd.DataFrame({'count_fuel_false': df01[(df01['show_in_report'] == 'f') & (df01['price'] > 0) &
												  (df01['date_upd'] == datetime.datetime.now().date()) &
												  (df01['source'].isin(
													  ['api_azsgo', 'api_benzuber', 'api_yandex_fuel', 'api_azsopti']))]
						.groupby(['station_id', 'source']).size()}).reset_index()
